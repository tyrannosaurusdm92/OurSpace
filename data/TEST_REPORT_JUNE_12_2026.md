...
- Replaced Onyx mood images with the transparent-background versions from the newer uploaded Onyx package.
- Set Onyx image rendering to `object-fit: contain` so transparent PNGs are not cropped.
...
- Added automatic rewards for saving DBT diary cards, saving journals, and using Onyx chat support when those linked tasks are due.
...
- Onyx chat responds to DBT/ADHD support messages.
...
- All 11 Onyx mood PNG files are RGBA transparent-background images.
...
## Serotonin + Onyx Lore Revision
- Replaced the Onyx personality source with the updated Momma-centered Lord Onyx Blepman lore.
