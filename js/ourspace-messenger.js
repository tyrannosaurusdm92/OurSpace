(function (global) {
  ...
  function readStorage(storageKey = STORAGE_KEY) {
    try { return JSON.parse(localStorage.getItem(storageKey) || '{}'); }
    catch (error) { return {}; }
  }

  function writeStorage(data, storageKey = STORAGE_KEY) {
  ...
  class MessengerApp {
    constructor(config) {
      ...
      this.storageKey = this.config.storageKey || STORAGE_KEY;
      this.storage = Object.assign(createDefaultState(this.config), readStorage(this.storageKey));
      ...
      const Bridge = global.OurSpaceBackendBridge || class {
        async sendMessage(){ return { ok:false, offline:true, queued:false, reason:'Backend bridge unavailable.' }; }
        async listMessages(){ return { ok:false, messages:[] }; }
        async createChannel(){ return { ok:false }; }
        async sendSignal(){ return { ok:false }; }
        async listSignals(){ return { ok:false, signals:[] }; }
        async sendOnyxAlert(){ return { ok:false }; }
        async flushQueue(){ return { ok:true, sent:0, remaining:0 }; }
      };
    ...
    save() {
      ...
      writeStorage(clean, this.storageKey);
    }
