(function(){
  ...
  function sendUserMessage(text){
    ...
    setMood('thinking');
    window.setTimeout(() => {
      let result = answer(trimmed);
      const backgroundScan = window.EmperorOnyxPsychiatrySupport?.scanForOnyx(trimmed) || null;
      if(backgroundScan && backgroundScan.extra){
        if(backgroundScan.priority === 'critical'){
          result = { mood:'caring', text: backgroundScan.extra.replace(/^\n\n/, '') + '\n\n' + result.text };
        } else if(!result.text.includes('Background support note:') && !result.text.includes('ADHD-aware next step:') && !result.text.includes('Body-care cue:')){
          result = { mood: backgroundScan.mood || result.mood, text: result.text + backgroundScan.extra };
        }
      }
      setMood(result.mood);
