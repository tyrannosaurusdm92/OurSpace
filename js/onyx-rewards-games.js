(function(){
  ...
  function initChatRewards(){
    const chatPairs = [
      ['onyxChatForm','onyxInput'],
      ['chatForm','chatInput'],
      ['floatingChatForm','floatingChatInput']
    ];
    chatPairs.forEach(([formId,inputId]) => {
      const form = $(formId);
      const input = $(inputId);
      if(form && input){
    ...
    document.querySelectorAll('[data-quick],[data-onyx]').forEach(btn => {
      btn.addEventListener('click', () => {
        const key = btn.dataset.quick || btn.dataset.onyx;
        const reward = quickRewards[key];
  ...
  function initGames(){
    ...
    function launchGame(){
      ...
      if(status) status.textContent = `Playing ${game.name}. In-game actions, score changes, levels, wins, and healthy game milestones now earn Jasper currency.`;
    ...
    if(claimStop) claimStop.addEventListener('click', () => addReward({ gold: 4 }, 'Used a regulated stop / save-progress pause', 'mobile_games', 'manual_game_claim'));
    const claimReturn = $('claimReturnToCare');
    if(claimReturn) claimReturn.addEventListener('click', () => addReward({ gold: 5 }, 'Used the game Home button and returned to care', 'mobile_games', 'manual_game_claim'));
