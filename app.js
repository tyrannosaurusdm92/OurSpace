(() => {
  ...
  function bindShell(){
    ...
    window.addEventListener("jasperCurrencyRewardLocal", handleGameCurrencyCustomEvent);
  ...
  function handleGameCurrencyMessage(event){
    ...
    applyGameCurrencyReward(data, event.source);
  }
  function handleGameCurrencyCustomEvent(event){
    const data=event.detail||{};
    if(data.type!=="jasperCurrencyReward") return;
    applyGameCurrencyReward(data, null);
  }
  function applyGameCurrencyReward(data, replyTarget){
    ...
    replyTarget?.postMessage?.({type:"jasperCurrencySync",amount:state.currency,totalCopper:currencyToCopper(state.currency)},"*");
  }
  ...
  function bindGames(){ window.SQUISHY_REWARDS_LOCAL_GAMES_ONLY=true; }
